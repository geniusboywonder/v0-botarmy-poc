...
export interface Agent {
  ...
  name: string
  role: string
  status: "active" | "idle" | "error" | "offline" | "thinking" | "paused" | "starting" | "working" | "completed" | "skipped"
  currentTask?: string
  lastActivity: Date
...
interface AgentStore {
  ...
  updateAgentStatus: (statusData: { agent_name: string; status: any; current_task: any; progress_percentage: any; error_message: any; estimated_completion: any; }) => void
      ...
      (set, get) => ({
        ...
        updateAgentStatus: (statusData) => {
          set((state) => ({
            agents: state.agents.map((agent) =>
              agent.name.toLowerCase() === statusData.agent_name.toLowerCase()
                ? {
                    ...agent,
        ...
        updateAgentFromMessage: (message) => {
          if (message.type === 'agent_status') {
            get().updateAgentStatus(message.data);
          }
        },
